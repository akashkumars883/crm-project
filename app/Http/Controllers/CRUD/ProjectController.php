...
class ProjectController extends Controller
{
    ...
    public function show(Project $project)
    {
        if (Auth::user()->hasPermission('read-project')) {
            ...
            $projectTasks = ProjectTask::with('assignee')->where('project_id', $projectId)->get();
            $assignees = User::all();

            return view('crm.crud.projects.show', compact('project', 'administrativeCost', 'totalLaborCost', 'result', 'totalCostIncurred', 'totalMaterialCost', 'totalMaterial', 'totalLabor', 'attachmentTypes', 'projects', 'customers', 'paymentStatuses', 'employees', 'inventories', 'paymentMethods', 'billStatuses', 'billTypes', 'activityTypes', 'contactMethods', 'attachments', 'activities', 'bills', 'payments', 'projectTasks', 'assignees'));
        } else {
            abort(403, 'Unauthorized Access');
        }       
    }
