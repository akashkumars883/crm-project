    ...
    <div class="row justify-content-center">
        <div class="col-sm-6 text-center">
            <div class="page-amount-box">
                <h4 class="page-amount">Edit Payment</h4>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-control" id="customer_id" name="customer_id">
                                ...
                                @foreach ($customers as $customer)
                                    <option value="{{ $customer->id }}"{{ $payment->customer_id == $customer->id ? ' selected' : '' }}>{{ $customer->lead->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="mb-3">
                            ...
                            <select class="form-control" id="project_id" name="project_id">
                                ...
                                @foreach ($projects as $project)
                                    <option value="{{ $project->id }}"{{ $payment->project_id == $project->id ? ' selected' : '' }}>{{ $project->id }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="mb-3">
                            <label for="amount" class="form-label">Amount</label>
                            <input type="text" class="form-control" id="amount" name="amount" value="{{ old('amount', $payment->amount) }}" required>
                        </div>
                        <div class="mb-3">
