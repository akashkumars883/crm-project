        ...
        <div class="col-12">
            <div class="card shadow-sm border-0">
                <div class="card-header bg-white py-3 border-bottom d-flex align-items-center">
                    ...
                    <h5 class="card-title mb-0 fw-bold">Edit Inventory Item #{{ $inventory->id }}</h5>
                </div>
                    ...
                    <form action="{{ route('inventories.update', $inventory->id) }}" method="POST">
                        ...
                        <div class="row g-3">
                            <div class="col-md-6">
                                ...
                                <input type="text" class="form-control" id="title" name="title" value="{{ $inventory->title }}" required>
                            </div>
                            <div class="col-md-3">
                                ...
                                <select class="form-select" id="inventory_type_id" name="inventory_type_id" required>
                                    ...
                                    @foreach ($inventoryTypes as $inventoryType)
                                        <option value="{{ $inventoryType->id }}"{{ $inventory->inventory_type_id == $inventoryType->id ? ' selected' : '' }}>{{ $inventoryType->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-3">
                                ...
                                <select class="form-select" id="inventory_status_id" name="inventory_status_id" required>
                                    ...
                                    @foreach ($inventoryStatuses as $inventoryStatus)
                                        <option value="{{ $inventoryStatus->id }}"{{ $inventory->inventory_status_id == $inventoryStatus->id ? ' selected' : '' }}>{{ $inventoryStatus->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="col-md-4">
                                ...
                                <input type="number" step="0.01" class="form-control" id="cost" name="cost" value="{{ $inventory->cost }}" required>
                            </div>
                            <div class="col-md-4">
                                ...
                                <select class="form-select" id="project_id" name="project_id">
                                    ...
                                    @foreach ($projects as $project)
                                        <option value="{{ $project->id }}"{{ $inventory->project_id == $project->id ? ' selected' : '' }}>{{ $project->customer->lead->name ?? 'Project #'.$project->id }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-4">
                                ...
                                <select class="form-select" id="vendor_id" name="vendor_id">
                                    ...
                                    @foreach ($vendors as $vendor)
                                        <option value="{{ $vendor->id }}"{{ $inventory->vendor_id == $vendor->id ? ' selected' : '' }}>{{ $vendor->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="col-12">
                                ...
                                <textarea class="form-control" id="description" name="description" rows="3">{{ $inventory->description }}</textarea>
                            </div>
                        </div>

                        <div class="mt-4 pt-3 border-top text-end">
                            <a href="{{ route('inventories.index') }}" class="btn btn-secondary me-2">Cancel</a>
                            <button type="submit" class="btn btn-primary px-4"><i class="ti ti-check"></i> Update Inventory</button>
                        </div>
                    </form>
                </div>
            </div>
