        ...
        <div class="col-12">
            <div class="card shadow-sm border-0">
                <div class="card-header bg-white py-3 border-bottom d-flex align-items-center">
                    ...
                    <h5 class="card-title mb-0 fw-bold">Log New Activity</h5>
                </div>
                    ...
                    <form action="{{ route('activities.store') }}" method="POST">
                        ...
                        <div class="row g-3">
                            <div class="col-md-6">
                                ...
                                <input type="text" class="form-control" id="title" name="title" value="{{ old('title') }}" placeholder="Title or summary of activity" required>
                            </div>
                            <div class="col-md-3">
                                ...
                                <select class="form-select" id="activity_type_id" name="activity_type_id" required>
                                    ...
                                    @foreach ($activityTypes as $activityType)
                                        <option value="{{ $activityType->id }}"{{ old('activity_type_id') == $activityType->id ? ' selected' : '' }}>{{ $activityType->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-3">
                            ...
                            <div class="col-md-4">
                                ...
                                <select class="form-select" id="customer_id" name="customer_id">
                                    ...
                                    @foreach ($customers as $customer)
                                        <option value="{{ $customer->id }}"{{ old('customer_id') == $customer->id ? ' selected' : '' }}>{{ $customer->lead->name ?? 'N/A' }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-4">
                                ...
                                <select class="form-select" id="project_id" name="project_id">
                                    ...
                                    @foreach ($projects as $project)
                                        <option value="{{ $project->id }}"{{ old('project_id') == $project->id ? ' selected' : '' }}>Project #{{ $project->id }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="col-12">
                                ...
                                <textarea class="form-control" id="description" name="description" rows="3" placeholder="Key outcomes or discussion notes...">{{ old('description') }}</textarea>
                            </div>
                        </div>

                        <div class="mt-4 pt-3 border-top text-end">
                            <a href="{{ route('activities.index') }}" class="btn btn-secondary me-2">Cancel</a>
                            <button type="submit" class="btn btn-primary px-4"><i class="ti ti-check"></i> Create Activity</button>
                        </div>
                    </form>
                </div>
            </div>
