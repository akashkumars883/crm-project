@extends('layouts.app')
@section('title', optional($customer->lead)->name ?? 'Customer Details')
@section('content')
                            ...
                            <div class="card-body">
                                <div class="media mb-3">
                                    <div class="media-body align-self-center">
                                        <h4 class="text-primary mt-0 font-24">{{ optional($customer->lead)->name ?? 'N/A' }} </h4>
                                        <p class="mb-0 font-14"><i class="fas fa-phone-square"></i> : {{ optional($customer->lead)->phone ?? 'N/A' }} </p>
                                        <p class="mb-0 font-14"><i class="fas fa-phone-square"></i> : {{ optional($customer->lead)->email ?? 'N/A' }} </p>
                                        <p class="mb-0 font-14"><i class="fas fa-home"></i> : {{ optional($customer->lead)->address ?? 'N/A' }}, {{ $customer->city ?? 'N/A' }}, {{ $customer->state ?? 'N/A' }} - {{ $customer->zip ?? 'N/A' }} </p>
                                    </div><!--end media body-->
                                </div> <!--end media-->

                                {{-- <p class="mb-0 font-12">Type : {{ $customer->customerType->name }} </p> --}}
                                {{-- <p class="mb-0 font-12">Company : {{ $customer->company_name ?? '-' }} </p> --}}
                                <p class="mb-0 font-14">Source : {{ optional(optional($customer->lead)->leadSource)->name ?? 'N/A' }} </p>
                                <hr class="hr-dashed">
                    ...
                    <div class="col-lg-4">
                        <div class="card">
                            <div class="card-body">
                                <div class="media mb-3">
                                    <div class="media-body align-self-center">
                                        <p class="mb-0 font-14">Contact Method : {{ optional(optional($customer->lead)->contactMethod)->name ?? 'Not Assigned' }}</p>
                                        <p class="mb-0 font-14">Contact Language : {{ optional(optional($customer->lead)->contactLanguage)->name ?? 'Not Assigned' }}</p>
                                        <p class="mb-0 font-14">Lead Notes : {{ optional($customer->lead)->notes ?? '-' }} </p>
                                    </div><!--end media body-->
                                </div> <!--end media-->

                                <p class="mb-0 font-14">Lead Since : {{ optional($customer->lead)->created_at ? $customer->lead->created_at->format('D, d M Y h:i A') : 'N/A' }} </p>
                            ...
                            <div class="card-body">
                                <div class="media mb-3">
                                    <div class="media-body align-self-center">
                                        ...
                                        <p class="mb-0 font-14">Lead Age : {{ optional($customer->lead)->created_at ? $customer->lead->created_at->diffInDays(now()) : 0 }}  days</p>
                                        <p class="mb-0 font-14">Conversion Age : {{ $customer->created_at->diffInDays(now()) }}  days</p>
                                        <p class="mb-0 font-14">Lead Status : {{ optional(optional($customer->lead)->leadStatus)->name ?? 'N/A' }}  since {{ optional(optional($customer->lead)->leadStatus)->updated_at ? $customer->lead->leadStatus->updated_at->diffInDays(now()) : 0 }}  days</p>
                                    </div><!--end media body-->
                                </div> <!--end media-->

                                <p class="mb-0 font-14">Assigned To : {{ optional(optional($customer->lead)->assignedTo)->name ?? 'Not Assigned' }}</p>
                                <p class="mb-0 font-14">Since : {{ optional(optional($customer->lead)->assignedTo)->created_at ? $customer->lead->assignedTo->created_at->diffInDays(now()) : 'Not Assigned' }} days </p>
                            </div><!--end card-body-->
                        </div>  <!--end card-->
                    </div><!--end col-->
                </div>
...
<div class="modal fade" id="projectModal" tabindex="-1" aria-labelledby="projectModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header bg-primary">
                <h5 class="modal-title text-white" id="projectModalLabel">Create Project for this {{ optional($customer->lead)->name ?? 'Customer' }}</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
...
<div class="modal fade" id="paymentModal" tabindex="-1" aria-labelledby="paymentModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header bg-primary">
                <h5 class="modal-title text-white" id="paymentModalLabel">Create Payment for this {{ optional($customer->lead)->name ?? 'Customer' }}</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
